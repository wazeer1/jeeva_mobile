...
export default function Status({navigation}){
    ...
    <ScrollView style={{backgroundColor:'#F3F8F9',paddingHorizontal:16}}>
        <View style={{paddingVertical:15}}>
            <Text style={{textAlign:'center',fontSize:24,fontWeight:'bold',color:'#000'}}>Quality Status</Text>
        </View>
        ...
        <ValueCard label="Average CLR" value={averages?.avg_total_clr_value?.toFixed(2)} icon={PaintBucket}/>
        <ValueCard label="Average FAT" value={averages?.avg_total_fat_value?.toFixed(2)} icon={PaintBucket}/>
        <ValueCard label="Average SNF" value={averages?.avg_total_snf_value?.toFixed(2)} icon={PaintBucket}/>
    </ScrollView>
    )
}
